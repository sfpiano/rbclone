...
class DiffSet(models.Model):
  ...
  def add_approval(self, id):
    self.approval_status += (",%d") % id
    count = 0
    total = 0
    for r in self.reviewer_ids.all():
      total += 1
      if self.approval_status.find(str(r.id)) >= 0:
        count += 1

    if count == total:
      self.status = self.APPROVED
    elif count > 0:
      self.status = self.PART_APPROV
  ...
  def get_status_html(self):
    if self.status == self.CREATED:
      return "#FFFFFF"
    elif self.status == self.PART_APPROV:
      return "#FFC956"
